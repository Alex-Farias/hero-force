...
import { z } from 'zod';
import { X, Save, Folder, FileText, Activity, AlertCircle, Edit2, Calendar, User as UserIcon } from 'lucide-react';
...
export function ProjectDetailsModal({ isOpen, onClose, onSave, project }: ProjectDetailsModalProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if(project) {
      reset({
        ...
        goals: project.goals,
        user: project.user?.id ? String(project.user.id) : '',
      });
      setIsEditing(false);
    }
          ...
          <div className="flex items-center gap-2">
            {!isEditing && (
              <button 
                ...
                className="p-2 text-muted hover:text-aux transition-colors rounded-lg hover:bg-secondary/10"
                title="Editar"
              >
                <Edit2 size={20} />
              </button>
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="p-6 space-y-6 overflow-y-auto">
          <div className={`grid grid-cols-1 ${isAdmin ? 'md:grid-cols-4' : 'md:grid-cols-3'} gap-4`}>
            <div className={`${isAdmin ? 'md:col-span-2' : 'md:col-span-2'} space-y-2`}>
              ...
              {isEditing ? (
                ...
                <p className="text-main font-medium text-lg">{project.name}</p>
              )}
            </div>
            ...
            {isAdmin && (
              <div className="space-y-2">
                ...
                {isEditing ? (
                  ...
                  <div className="w-full bg-secondary/10 border border-secondary/20 rounded-lg px-4 py-2.5 text-main opacity-70 cursor-not-allowed">
                    {project.user?.name || 'Desconhecido'}
                  </div>
                )}
              </div>
            )}
          ...
          <div className="space-y-2">
            ...
            {isEditing ? (
              ...
              <p className="text-main leading-relaxed bg-secondary/5 p-4 rounded-lg border border-secondary/10 break-words whitespace-pre-wrap">
                {project.description}
              </p>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            {isEditing ? (
              ...
              <p className="text-main leading-relaxed bg-secondary/5 p-4 rounded-lg border border-secondary/10 break-words whitespace-pre-wrap">
                {project.goals || 'Nenhuma meta definida.'}
              </p>
            )}
          </div>
          ...
          {!isEditing && (
             <div className="space-y-2">
                <label className="text-sm font-medium text-muted flex items-center gap-2">
                  <Calendar size={16} /> Data de Criação
                </label>
                <p className="text-main">{new Date(project.createdAt).toLocaleDateString()}</p>
             </div>
          )}
