...
export class ProjectService {
  ...
  async findAll(currentUser: UserResponseDto): Promise<ProjectResponseDto[]> {
    ...
    if ((await this.checkPermission(currentUser)) === false) {
      where.user = { id_user: currentUser.id };
    }
  ...
  async create(
    ...
    const finalId =
      (await this.checkPermission(currentUser)) && dto.user
        ? dto.user
        : currentUser.id;
  ...
  async update(
    ...
    const oldProject = await this.projectRepository.findOneBy({
      id_project: id,
      isActive: true,
    });
    if (!oldProject) {
  ...
  async remove(
    ...
    if ((await this.checkPermission(currentUser, userId)) === false) {
      throw new Error('User not authorized');
    }
